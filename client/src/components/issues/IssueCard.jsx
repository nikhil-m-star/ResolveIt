      ...
      <div className="p-6 flex flex-col gap-5 relative z-10">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">#{issue.id.slice(-5).toUpperCase()}</span>
          </div>

          <Link to={`/issues/${issue.id}`} className="block">
            <h3 className="line-clamp-2 font-heading text-xl font-black text-white group-hover:text-primary transition-colors leading-[1.1] uppercase tracking-tight">
              {issue.title}
            </h3>
          </Link>
        ...
        <div className="flex items-center justify-between pt-5 border-t border-white/5">
           ...
           <div className="flex items-center gap-2.5 px-3 py-1.5 bg-white/5 rounded-xl border border-white/5 text-[9px] font-black text-slate-500 uppercase tracking-widest group-hover:border-primary/20 transition-all">
              <Activity className="h-3.5 w-3.5 text-primary" /> Sector Verified
           </div>
        </div>
      </div>
    </Motion.div>
